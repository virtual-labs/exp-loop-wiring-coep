...
function Diagram22(){
    ...
    $("#Header").html("<center><span>DISTILATION DI SINKING</span></center>");
//    alert("Dhanashree");
...
//    
//    paper.text(x-18,y-70,"DI").attr({'stroke' : '#5c696e' , "font-size":"14px"});
    
    const dropZones = [
        { x: x + 370, y: y - 35, width: 45, height: 42, type: "LSH", zoneId: "LSH1", occupied: false },
        { x: x + 370, y: y + 25, width: 45, height: 42, type: "LSH", zoneId: "LSH2", occupied: false },
        { x: x + 370, y: y + 85, width: 45, height: 42, type: "LSH", zoneId: "LSH3", occupied: false },
        { x: x + 370, y: y + 145, width: 45, height: 42, type: "LSH", zoneId: "LSH4", occupied: false },
        { x: x + 60, y: y - 100, width: 45, height: 42, type: "NEGPOS", zoneId: "NEGPOS1", occupied: false },
        { x: x -70, y: y -60, width: 100, height: 255, type: "CARD", zoneId: "CARD1", occupied: false } // For this I want drop on this block only only for this 
    ];
...
// Valid card types
const cardTypes = ["CARD", "CARD1"];

// Helper: bidirectional type and port match
const isBidirectionalMatch = (typeA, nameA, typeB, nameB) => (
    (fromType === typeA && fromName === nameA && toType === typeB && toName === nameB) ||
    (toType === typeA && toName === nameA && fromType === typeB && fromName === nameB)
);

// CARD <-> I5P, I6P, I7P
const isCardToI5P = (
    (cardTypes.includes(fromType) && fromName === "three" && toType === "I5P" && toName === "bottom") ||
    (cardTypes.includes(toType) && toName === "three" && fromType === "I5P" && fromName === "bottom")
);

const isCardToI6P = (
    (cardTypes.includes(fromType) && fromName === "four" && toType === "I6P" && toName === "bottom") ||
    (cardTypes.includes(toType) && toName === "four" && fromType === "I6P" && fromName === "bottom")
);

const isCardToI7P = (
    (cardTypes.includes(fromType) && fromName === "five" && toType === "I7P" && toName === "bottom") ||
    (cardTypes.includes(toType) && toName === "five" && fromType === "I7P" && fromName === "bottom")
);
...
// NEGPOSP.bottom ⇄ I8P.top in NEGPOS2
 else if (
    (
    ...
    const requirements = [
        {
            zoneId: "LSH1",
            requirements: [
                { from: { type: "LSH", port: "top" }, to: { type: "LSHSP1", port: "top" } },
                { from: { type: "LSH", port: "bottom" }, to: { type: "LSHSP2", port: "bottom" } }
            ]
        },
        {
            zoneId: "LSH2",
            requirements: [
                { from: { type: "LSH", port: "top" }, to: { type: "LSHS2P2", port: "top" } },
                { from: { type: "LSH", port: "bottom" }, to: { type: "LSHS2P3", port: "bottom" } }
            ]
        },
        {
            zoneId: "LSH3",
            requirements: [
                { from: { type: "LSH", port: "top" }, to: { type: "LSHS3P1", port: "top" } },
                { from: { type: "LSH", port: "bottom" }, to: { type: "LSHS3P2", port: "bottom" } }
            ]
        },
        {
            zoneId: "LSH4",
            requirements: [
                { from: { type: "LSH", port: "top" }, to: { type: "LSHS4P1", port: "top" } },
                { from: { type: "LSH", port: "bottom" }, to: { type: "LSHS4P2", port: "bottom" } }
            ]
        },
        {
            zoneId: "NEGPOS1",
        ...
        {  
            zoneId: "CARD1",
            requirements: [
                { from: { type: "CARD", port: "one" }, to: { type: "NEGPOS", port: "right" } },
                { from: { type: "CARD", port: "two" }, to: { type: "NEGPOSP", port: "top" } },
                { from: { type: "CARD", port: "three" }, to: { type: "I5P", port: "bottom" } },
                { from: { type: "CARD", port: "four" }, to: { type: "I6P", port: "bottom" } },
                { from: { type: "CARD", port: "five" }, to: { type: "I7P", port: "bottom" } }
            ]
        },
        {  
            zoneId: "I3P2",
...
//	  alert(" All required connections are correct! ");
	  ...
	  var htm = `<img src='images/sprayDryerDISinking.png' class='img-fluid' 
        ...
        if(finCnt>=4 && diCnt>=2){
			...
			$("#subhookUpSelect").prop("hidden",true);	    	
		 $("#subhookUpSelectLabel").prop("hidden",true);
       ...
       $("#counter").prop("hidden",true); 
	  if(diCnt<2){
		$("#subhookUpSelect").prop("disabled",false);
		$("#hookUpSelect").prop("disabled",true);
		
		
	}else{
		$("#subhookUpSelect").prop("disabled",false);
	    	$("#subhookUpSelect").val(0);
		 $("#subhookUpSelectLabel").prop("hidden",true);
        $("#subhookUpSelect").prop("hidden",true);
...
//		alert("image");
         Swal.fire({
					...
					html: `<div>
                <img src='images/sprayDryerDISinking.png' class='img-fluid' 
