...
function aoSpray() {
    ...
    $("#Header").html("<center><span>SPRAY DRYER AO</span></center>");
    
    $("#startBtnAO").prop("hidden",false);
...
//    paper.image("images/fuse.png", x + 200, y - 50, 100, 250);
    ...
    paper.image("images/staticPN.png", x -70, y - 108, 101, 50);
    paper.image("images/AOLabel.png", x+38, y - 95, 23, 20);
...
//    paper.text(x-18,y-70,"AO").attr({'stroke' : '#5c696e' , "font-size":"14px"});
    
    const dropZones = [
        { x: x + 250, y: y - 50, width: 45, height: 42, type: "FY", zoneId: "LSH1", occupied: false },
        { x: x + 250, y: y + 10, width: 45, height: 42, type: "FY", zoneId: "LSH2", occupied: false },
        { x: x + 250, y: y + 75, width: 45, height: 42, type: "FY", zoneId: "LSH3", occupied: false },
        { x: x + 250, y: y + 139, width: 45, height: 42, type: "FY", zoneId: "LSH4", occupied: false },
...
//            const portOffsets = portConfigurations[draggedType] || [];
  ...
  document.getElementById("startBtnAO").addEventListener("click", () => {
   mimicCount5++;
   const requirements = [
    {
        zoneId: "CARDA1",
        requirements: [
            ...
            {
                from: { type: "FY", port: "bottom" },
                to: { type: "CARDA", port: "zeroNegRight" }
            },
            {
                from: { type: "CARDA", port: "onePosRight" },
                to: { type: "FY", port: "top" }
            },
            {
                from: { type: "FY", port: "bottom" },
            ...
            {
                from: { type: "FY", port: "top" },
                to: { type: "CARDA", port: "twoPosRight" }
            },
            {
                from: { type: "FY", port: "bottom" },
            ...
            {
                from: { type: "FY", port: "top" },
                to: { type: "CARDA", port: "threePosRight" }
            },
            {
                from: { type: "FY", port: "bottom" },
    ...
    if (allValid) {
	  ...
	  $("#startBtnAO").prop("hidden",true);
	  $("#hookUpSelect").prop("disabled",false);
	  
	  $("#diagram").html("");
	  
	  resultJson.AoBoiler = mimicCount5;
        console.log(resultJson);
        
        timerMasterJson.AoBoiler = $("#counter").text();
		console.log(timerMasterJson);
		seconds = 0;
		updateCounter();
	  
	  var htm = `<img src='images/sprayAO.png' class='img-fluid' 
        ...
        Swal.fire({
            ...
            customClass: {
	         ...
	         Swal.fire({
					...
					html: `<div>
                <img src='images/sprayAO.png' class='img-fluid' 
